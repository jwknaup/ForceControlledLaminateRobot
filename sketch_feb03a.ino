...
void encAngleUpdate(short jointNum){
  const unsigned char countsPerEdgePerChannel = 3, numChannels = 2, numEdges = 1, gearRatio = 100;
  const unsigned short countsPerRev = countsPerEdgePerChannel*numChannels*numEdges*gearRatio;
  const float ticksPerDegree = (float) countsPerRev/360;
...
void loop() {
  ...
  // Serial.println(Joint[0].encAngle);
